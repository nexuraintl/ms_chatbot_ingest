...
- ⚠️ **Conocido: `POST /events/gcs` dispara la alerta de proyecto "Cloud Run — Latencia p95 degradada" (>3000ms sostenido 5 min)** — no es un bug, es esperado por diseño: `import_gcs_object()` (`api/services/ingestion_service.py`) hace polling sincrónico contra Gemini (`_IMPORT_POLL_INTERVAL_SECONDS=2`, hasta 30 intentos) dentro del mismo request HTTP antes de responderle a Eventarc; ya se midieron 3.1s/3.9s reales solo con un archivo de prueba de 124 bytes (2026-07-31). Como el servicio tiene tráfico muy esporádico, un par de eventos de alta ya alcanzan para mover el p95 — cualquier ingesta real de un tenant nuevo (ej. floridablanca, 21 documentos) va a disparar varias alertas. Decisión explícita: no tocar por ahora (ver conversación 2026-07-31). Alternativas evaluadas si se vuelve molesto: excluir este servicio de esa política de alerta / darle un umbral propio, o rearquitecturar para responder 204 de inmediato y hacer el import+poll en background.
